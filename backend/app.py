...
app = Flask(__name__)
# Updated CORS for production - allows all origins (adjust as needed for security)
...
def update_beach(mapbox_id):
    data = request.json
    result = supabase.table("beaches").update(data).eq('mapbox_id', mapbox_id).execute()  # Fixed variable name
